...
Commandline arguments:
    ...
    --expected-ttft-gain: Expected minimum speed-up in time-to-first-token
                         (warmup/query) as a factor, e.g. 4.3 for 4.3x. If
                         actual gain is below this, exits nonzero.

    --expected-latency-gain: Expected minimum speed-up in total round time
                            (warmup/query) as a factor, e.g. 4.5 for 4.5x.
                            If actual gain is below this, exits nonzero.
...
def visualize_results(warmup_df, benchmark_df):
    ...
    plot_bars(warmup_df, "Warmup Round", OUTPUT_DIR / "warmup_round.png")
    plot_bars(benchmark_df, "Query Round", OUTPUT_DIR / "query_round.png")
...
async def main(args):
    ...
    # Per-run salt keeps this benchmark repeatable against a persistent
    # KV cache (LMCache survives engine restarts): without it, a second run
    # replays byte-identical documents, the "cold" warmup round hits the
    # previous run's cache, and the gain gate measures nothing.
    salt = args.prompt_salt or str(int(time.time()))
    pre_warmup_prompts = [
        f"{salt} {i}xx " + " ".join(["hi"] * 1000) for i in range(5)
    ]
    ...
    # Prepare the prompts:
    # we append the salt + document id at the beginning to avoid any of the
    # document being the prefix of other documents (or of a previous run)
    warmup_prompts = [
        f"{salt} {i} " + " ".join(["hi"] * args.document_length)
    ...
    warmup_df.to_csv(OUTPUT_DIR / "warmup_round.csv", index=False)
    benchmark_df.to_csv(OUTPUT_DIR / "query_round.csv", index=False)
    ...
    # Gates: enforce the expected speed-ups documented above. A cache-less
    # (or broken-cache) server yields gains ~1.0 and fails here.
    failures = []
    ttft_gain = warmup_mean_ttft / query_mean_ttft if query_mean_ttft else 0.0
    latency_gain = (
        warmup_round_time_per_prompt / query_round_time_per_prompt
        if query_round_time_per_prompt
        else 0.0
    )
    if args.expected_ttft_gain is not None and ttft_gain < args.expected_ttft_gain:
        failures.append(
            f"TTFT gain {ttft_gain:.2f}x below expected {args.expected_ttft_gain}x"
        )
    if (
        args.expected_latency_gain is not None
        and latency_gain < args.expected_latency_gain
    ):
        failures.append(
            f"latency gain {latency_gain:.2f}x below expected "
            f"{args.expected_latency_gain}x"
        )
    if failures:
        for failure in failures:
            print(f"GATE FAILED: {failure}", file=sys.stderr)
        sys.exit(1)
    if args.expected_ttft_gain is not None or args.expected_latency_gain is not None:
        print(f"GATES PASSED: ttft gain {ttft_gain:.2f}x, "
              f"latency gain {latency_gain:.2f}x")
...
def create_argument_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default=".",
        help="Directory for csv/png artifacts (default: current directory).",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Prefix baked into every document to make this run's prompts "
            "unique against a persistent KV cache. Default: unix timestamp. "
            "Pass a fixed value to deliberately replay a previous run."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Minimum required TTFT speed-up (warmup/query) as a factor, "
            "e.g. 1.5 for 1.5x. Exits nonzero when the actual gain is below."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Minimum required round-time speed-up (warmup/query) as a factor. "
            "Exits nonzero when the actual gain is below."
        ),
    )
