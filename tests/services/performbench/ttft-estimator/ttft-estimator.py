...
def parse_args():
    ...
    parser.add_argument(
        "--base-url", type=str, required=False, default=None,
        help="Full base URL incl. /v1; overrides --host/--port.",
    )
    parser.add_argument(
        "--output-dir", type=str, required=False, default=".",
        help="Directory for the prefill-estimation.png plot.",
    )
...
def draw_quadratic_interpolation():
    ...
    plt.savefig(Path(args.output_dir) / "prefill-estimation.png")
...
if __name__ == "__main__":
    ...
    client = OpenAI(
        api_key=os.getenv("OPENAI_API_KEY", "dummy-key"), base_url=base_url
    )
