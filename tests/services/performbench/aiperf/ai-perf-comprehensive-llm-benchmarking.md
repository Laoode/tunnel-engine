...
**Tool**: [AIPerf v0.5.0](https://github.com/ai-dynamo/aiperf) | [Architecture Overview](/aiperf/architecture-internals/architecture-of-ai-perf) | [Full Documentation](/aiperf/welcome-to-ai-perf-documentation)
...
- ✅ [Server-side metrics collection](/aiperf/server-metrics/server-metrics-collection) via Prometheus
- ✅ [Automatic plot generation](/aiperf/tutorials/metrics-analysis/visualization-and-plotting-with-ai-perf) (`aiperf plot` command)
- ✅ [KV cache efficiency testing](/aiperf/tutorials/datasets-inputs/prefix-data-synthesis-tutorial) with trace synthesis
- ✅ [User-centric timing mode](/aiperf/tutorials/metrics-analysis/user-centric-timing-for-kv-cache-benchmarking) for multi-turn KV cache TTL testing
- ✅ [Goodput analysis](/aiperf/tutorials/metrics-analysis/benchmark-goodput-with-ai-perf) for SLA compliance measurement
- ✅ [Time-sliced analysis](/aiperf/tutorials/metrics-analysis/time-slicing-for-performance-analysis) for performance trends over time

**📚 Documentation**: See the full [CLI Options Reference](/aiperf/reference/command-line-options) for all available parameters.
...
AIPerf outputs detailed per-request data in `profile_export.jsonl`. Each line is a JSON record. See the [Working with Profile Exports](/aiperf/tutorials/metrics-analysis/working-with-profile-export-files) tutorial for more analysis techniques.
...
**📚 Documentation**: See [Benchmark Datasets](/aiperf/reference/benchmark-datasets) for supported dataset formats and [Trace Replay Mode](/aiperf/benchmark-modes/trace-replay-with-mooncake-traces) for detailed configuration.
...
**📚 Documentation**: See the [Timing Modes Reference](/aiperf/benchmark-modes/load-generator-options-reference) for all supported timing modes.
...
**Pro tip**: Use `--fixed-schedule` for end-to-end system validation (respects timing), or remove it to stress-test maximum throughput capacity. See the [Fixed Schedule Tutorial](/aiperf/tutorials/load-patterns-scheduling/fixed-schedule-benchmarking) for more details.
...
**📚 Documentation**: See the [Goodput Tutorial](/aiperf/tutorials/metrics-analysis/benchmark-goodput-with-ai-perf) for additional examples and SLO configuration options.
...
**📚 Documentation**: See the [Timeslices Tutorial](/aiperf/tutorials/metrics-analysis/time-slicing-for-performance-analysis) for configuration options and the [Warmup Tutorial](/aiperf/tutorials/load-patterns-scheduling/warmup-phase-configuration) for managing cold-start effects.
...
Simulate real-world user behavior where requests are cancelled mid-flight (e.g., users navigating away, timeouts). See the [Request Cancellation Tutorial](/aiperf/tutorials/load-patterns-scheduling/request-cancellation-testing) for detailed configuration.
...
AIPerf can collect server-side metrics from Prometheus endpoints exposed by your inference server (e.g., vLLM, TensorRT-LLM). See the [Server Metrics Guide](/aiperf/server-metrics/server-metrics-collection) for detailed configuration and the [Server Metrics Reference](/aiperf/server-metrics/ai-perf-server-metrics-reference) for supported metrics.
...
- **GPU telemetry**: See the [GPU Telemetry Tutorial](/aiperf/tutorials/metrics-analysis/gpu-telemetry-with-ai-perf) for DCGM integration

### Automatic Plot Generation

Generate visualizations from your profiling results with the `aiperf plot` command. See the [Plot Tutorial](/aiperf/tutorials/metrics-analysis/visualization-and-plotting-with-ai-perf) for all available plot types and configuration options.
...
Test prefix caching and KV cache reuse patterns with trace synthesis and user-centric timing. See the [Prefix Synthesis Tutorial](/aiperf/tutorials/datasets-inputs/prefix-data-synthesis-tutorial) for detailed configuration.
...
Simulate realistic multi-turn conversations with controlled per-user timing for KV cache TTL testing. See the [User-Centric Timing Tutorial](/aiperf/tutorials/metrics-analysis/user-centric-timing-for-kv-cache-benchmarking) for detailed configuration.
